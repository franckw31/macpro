...
// Tri des joueurs : 'jetons' (défaut) ou 'pseudo'
$sortBy = (isset($_GET['sort']) && $_GET['sort'] === 'pseudo') ? 'pseudo' : 'jetons';
$activities = [];
$players = [];
$seatingTables = [];
$flatSeating = [];
...
$selectedActivityId = 0; // pas d'activité sélectionnée, on affiche les joueurs avec jetons_2 > 1
$activityType = 3; // Mode finale : afficher les jetons_2

// Récupère tous les joueurs ayant jetons_2 > 1 (sans filtrer par activité)
if (true) {
	// Config activité (pour récupérer nb_tables)
	$sqlActConf = mysqli_query(
		$con,
		"SELECT `id-activite`, `titre-activite`, `nb-tables` AS nb_tables FROM `activite` LIMIT 1"
	);
	...
	// Récupère tous les joueurs avec jetons_2 > 1 qui ont participé aux 3 activités (filtrées par date_depart)
	$players = [];
	
	// D'abord, récupérer tous les membres avec jetons_2 > 1
	// Tri: par pseudo ou par jetons_2
	$orderByClause = ($sortBy === 'pseudo') ? 'ORDER BY pseudo ASC' : 'ORDER BY `jetons_2` DESC, pseudo ASC';
	
	$sqlMembers = mysqli_query($con, 
		"SELECT `id-membre`, pseudo, `jetons_1`, `jetons_2` 
		 FROM membres 
		 WHERE `jetons_2` > 1 
		 $orderByClause"
	);
	...
	if ($sqlMembers) {
		while ($memberRow = mysqli_fetch_assoc($sqlMembers)) {
			$membreId = (int)$memberRow['id-membre'];
			
			// Pour ce membre, récupérer sa participation aux 3 activités (par date_depart)
			$sqlPart = mysqli_query($con,
				"SELECT p.`id-participation`, p.`id-activite`, p.`ordre`, p.`option`, p.`id-table`, p.`id-siege`, p.`a_paye`, 
				        DATE_FORMAT(a.`date_depart`, '%Y-%m-%d') as ds, a.`titre-activite`
				 FROM participation p
				 JOIN activite a ON p.`id-activite` = a.`id-activite`
				 WHERE p.`id-membre` = $membreId 
				   AND DATE_FORMAT(a.`date_depart`, '%Y-%m-%d') IN ('2026-02-19', '2026-02-21', '2026-02-27')
				 ORDER BY p.`id-participation` DESC
				 LIMIT 1"
			);
			
			$playerRow = $memberRow;
			if ($sqlPart && mysqli_num_rows($sqlPart) > 0) {
				$partData = mysqli_fetch_assoc($sqlPart);
				$playerRow['id-participation'] = (int)$partData['id-participation'];
				$playerRow['id-activite'] = (int)$partData['id-activite'];
				$playerRow['ordre'] = (int)$partData['ordre'];
				$playerRow['option'] = $partData['option'];
				$playerRow['id-table'] = (int)$partData['id-table'];
				$playerRow['id-siege'] = (int)$partData['id-siege'];
				$playerRow['a_paye'] = (int)$partData['a_paye'];
				$playerRow['ds'] = $partData['ds'];
				$playerRow['titre-activite'] = $partData['titre-activite'];
			} else {
				$playerRow['id-participation'] = 0;
				$playerRow['id-activite'] = 0;
				$playerRow['ordre'] = 0;
				$playerRow['option'] = '';
				$playerRow['id-table'] = 0;
				$playerRow['id-siege'] = 0;
				$playerRow['a_paye'] = 0;
				$playerRow['ds'] = '';
				$playerRow['titre-activite'] = '';
			}
			$players[] = $playerRow;
		}
	}
	...
	if (isset($_GET['release_all_seats']) && $_GET['release_all_seats'] === '1') {
		file_put_contents('/tmp/release_all_seats_log.txt', date('Y-m-d H:i:s') . ' - release_all_seats détecté' . PHP_EOL, FILE_APPEND);
		file_put_contents('/tmp/release_all_seats_log.txt', 'Finale mode: releasing all seats' . PHP_EOL, FILE_APPEND);
		...
		$redirectUrl = 'finale.php?mode=' . htmlspecialchars($mode, ENT_QUOTES, 'UTF-8') . '&equilibrage=' . (int)$autoBalance . '&sort=' . $sortBy;
	...
	if ($totalPlayers > 0) {
		...
		if ($mode === 'sequential') {
			...
			if (!isset($_SESSION['sequential_order'][0]) || !is_array($_SESSION['sequential_order'][0])) {
				$_SESSION['sequential_order'][0] = [];
			}
			...
			if (isset($_GET['arrival_pid'])) {
				...
				if ($arrivalPid > 0 && !in_array($arrivalPid, $_SESSION['sequential_order'][0], true)) {
					$_SESSION['sequential_order'][0][] = $arrivalPid;
				}
			}

			// Annule la dernière arrivée enregistrée
			if (isset($_GET['seq_undo']) && $_GET['seq_undo'] === '1') {
				if (!empty($_SESSION['sequential_order'][0])) {
					// Nombre d'entrées avant suppression (pour distinguer le cas où
					// seule l'entrée de l'organisateur est présente sur ce poste).
					$beforeCount = count($_SESSION['sequential_order'][0]);
					$removedPid = array_pop($_SESSION['sequential_order'][0]);
			...
			$sequentialOrder = $_SESSION['sequential_order'][0];
			...
			if ($organizerParticipationIdSequential !== null && $organizerParticipationIdSequential > 0) {
				...
				$_SESSION['sequential_order'][0] = $sequentialOrder;
			}
		...
		if (!isset($_SESSION['paid_status'][0]) || !is_array($_SESSION['paid_status'][0])) {
			$_SESSION['paid_status'][0] = [];
		}
			...
			if ($pid > 0) {
				...
				if (isset($_GET['paid_pid'])) {
					...
					if ($paidPid > 0) {
						...
						$newPaidStatus = 0;
						if (isset($_SESSION['paid_status'][0][$paidPid]) && $_SESSION['paid_status'][0][$paidPid]) {
							unset($_SESSION['paid_status'][0][$paidPid]);
							$newPaidStatus = 0;
						} else {
							$_SESSION['paid_status'][0][$paidPid] = 1;
							$newPaidStatus = 1;
						}
						...
						header("Location: " . "finale.php?mode=" . $mode . "&equilibrage=" . $autoBalance . "&sort=" . $sortBy);
						exit;
					}
				}

		// Réinitialiser tous les statuts "A Payé"
		if (isset($_GET['reset_paid']) && $_GET['reset_paid'] === '1') {
			mysqli_query(
				$con,
				"UPDATE `participation` SET `a_paye` = 0 WHERE `id-participation` IN (SELECT `id-participation` FROM `participation` WHERE `id-membre` IN (SELECT `id-membre` FROM `membres` WHERE `jetons_2` > 1))"
			);
			$_SESSION['paid_status'][0] = [];
			// Redirection pour rafraîchir la page
			header("Location: " . "finale.php?mode=" . $mode . "&equilibrage=" . $autoBalance . "&sort=" . $sortBy);
	...
	// Endpoint pour le polling des changements de paiement
		if (isset($_GET['check_payments']) && $_GET['check_payments'] == 1) {
			...
			$sqlCurrentPayments = mysqli_query(
				$con,
				"SELECT `id-participation`, `a_paye` FROM `participation` WHERE `a_paye` = 1"
			);
		...
		if ($mode === 'auto' && $autoBalance === 0) {
			...
			for ($t = 0; $t < $tablesCount; $t++) {
				...
				for ($s = 0; $s < $tableSize; $s++) {
					if (isset($seatingTables[$t][$s]) && isset($seatingTables[$t][$s]['id-participation']) && $seatingTables[$t][$s]['id-participation']) {
						$perTableCounts[$t]++;
						if (isset($seatingTables[$t][$s]['jetons_2'])) {
							$perTableJetons[$t] += (int)$seatingTables[$t][$s]['jetons_2'];
						}
					} else {
			...
			if ($unseatedTotal > 0) {
				...
				usort($unseatedIndexes, function($idxA, $idxB) use ($players) {
					$jetonsA = isset($players[$idxA]['jetons_2']) ? (int)$players[$idxA]['jetons_2'] : 0;
					$jetonsB = isset($players[$idxB]['jetons_2']) ? (int)$players[$idxB]['jetons_2'] : 0;
				...
				while ($idxCursor < $unseatedTotal) {
					...
					if (isset($players[$playerIdx]['jetons_2'])) {
						$perTableJetons[$bestTable] += (int)$players[$playerIdx]['jetons_2'];
					}
				}
			}
	...
	// Trie le tableau récapitulatif selon le paramètre $sortBy
	if (!empty($flatSeating)) {
		usort($flatSeating, function($a, $b) {
			global $sortBy;
			
			if ($sortBy === 'pseudo') {
				// Trier par pseudo (alphabétique), puis par jetons décroissant
				$pa = isset($a['pseudo']) ? mb_strtolower($a['pseudo'], 'UTF-8') : '';
				$pb = isset($b['pseudo']) ? mb_strtolower($b['pseudo'], 'UTF-8') : '';
				if ($pa !== $pb) {
					return $pa <=> $pb; // Alphabétique
				}
				// Si même pseudo, trier par jetons décroissant
				$jetonsA = isset($a['jetons_2']) ? (int)$a['jetons_2'] : 0;
				$jetonsB = isset($b['jetons_2']) ? (int)$b['jetons_2'] : 0;
				return $jetonsB <=> $jetonsA;
			} else {
				// Par défaut: trier par jetons (décroissant), puis par pseudo alphabétique
				$jetonsA = isset($a['jetons_2']) ? (int)$a['jetons_2'] : 0;
				$jetonsB = isset($b['jetons_2']) ? (int)$b['jetons_2'] : 0;
				if ($jetonsA !== $jetonsB) {
					return $jetonsB <=> $jetonsA; // Décroissant
				}
				// Si même nombre de jetons, trier par pseudo
				$pa = isset($a['pseudo']) ? mb_strtolower($a['pseudo'], 'UTF-8') : '';
				$pb = isset($b['pseudo']) ? mb_strtolower($b['pseudo'], 'UTF-8') : '';
				return $pa <=> $pb;
			}
	...
	$sqlRefreshSeating = mysqli_query(
		$con,
		"SELECT `id-participation`, `id-table`, `id-siege` FROM `participation` WHERE `id-membre` IN (SELECT `id-membre` FROM `membres` WHERE `jetons_2` > 1)"
	);
...
<html lang="fr">
<head>
	<title>Admin | Finale - Tables de poker</title>
	<meta charset="utf-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=0, minimum-scale=1.0, maximum-scale=1.0">
	<script>
		// Rafraîchissement conditionnel : on interroge périodiquement le serveur
		// pour savoir si l'état des tables a changé pour les joueurs avec jetons_2 > 1.
		(function() {
			var lastVersion = null;
			function checkUpdate() {
				var xhr = new XMLHttpRequest();
				xhr.open('GET', 'tables_ping.php?finale_mode=1', true);
				...
				<section id="page-title">
					<div class="row">
						<div class="col-sm-12 text-center">
							<h1 class="mainTitle">Finale - Placement des Joueurs (Jetons 2 > 1)</h1>
						</div>
								...
								<div class="panel-body">
									<div class="poker-wrapper">
										...
										<?php if (empty($flatSeating)): ?>
											<p>Aucun joueur trouvé avec jetons_2 > 1.</p>
										<?php else: ?>
												<?php $tableSize = 9; ?>
														...
														<div class="poker-table-center <?php echo $colorClass; ?>" data-table="<?php echo $tableIndex + 1; ?>" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 5px;">
															...
															<?php
																...
																foreach ($tablePlayers as $player) {
																	if (isset($player['jetons_2']) && isset($player['id-participation']) && $player['id-participation']) {
																		$tableJetonsTotal += (int)$player['jetons_2'];
																	}
																}
																	...
																	<?php if ($player): ?>
																		<?php 
																			...
																			if (isset($activityType)) {
																				if ($activityType == 2 && isset($player['jetons_1']) && (int)$player['jetons_1'] != 0) {
												...
												<?php 
													...
													$jetonsList = [];
													foreach ($flatSeating as $player) {
														if (isset($player['jetons_2'])) {
															$jetonsList[] = (int)$player['jetons_2'];
														}
													}
														...
														<tbody>
															<?php foreach ($flatSeating as $player): ?>
																...
																<tr data-participation-id="<?php echo $pid; ?>">
																	...
																	<td><?php echo htmlspecialchars($player['ds']); ?></td>
																	...
																	<?php if ($mode === 'sequential'): ?>
																		...
																		<td>
																			<?php if (!isset($arrivalOrderMap[$pid])): ?>
																				<a href="finale.php?mode=sequential&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=<?php echo $sortBy; ?>&amp;arrival_pid=<?php echo $pid; ?>" class="btn btn-xs btn-primary">Arrivé</a>
																			<?php else: ?>
																	...
																	<td>
																		<input type="radio"
																		       ...
																		       onclick="window.location='finale.php?mode=<?php echo htmlspecialchars($mode, ENT_QUOTES, 'UTF-8'); ?>&equilibrage=<?php echo (int)$autoBalance; ?>&sort=<?php echo $sortBy; ?>&paid_pid=<?php echo $pid; ?>';" />
																	</td>
																</tr>
															<?php endforeach; ?>
														</tbody>
												...
												<div class="alert alert-info" style="max-width:900px;margin:10px auto 0;">
													<strong>Tri des joueurs :</strong>
													<a href="finale.php?mode=<?php echo htmlspecialchars($mode, ENT_QUOTES, 'UTF-8'); ?>&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=jetons" class="btn <?php echo ($sortBy === 'jetons') ? 'btn-primary' : 'btn-default'; ?> btn-sm">Par jetons</a>
													<a href="finale.php?mode=<?php echo htmlspecialchars($mode, ENT_QUOTES, 'UTF-8'); ?>&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=pseudo" class="btn <?php echo ($sortBy === 'pseudo') ? 'btn-primary' : 'btn-default'; ?> btn-sm">Par pseudo</a>
												</div>

												<div class="alert alert-info" style="max-width:900px;margin:10px auto 0;">
													<a href="finale.php?mode=<?php echo htmlspecialchars($mode, ENT_QUOTES, 'UTF-8'); ?>&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=<?php echo $sortBy; ?>&amp;reset_paid=1" class="btn btn-warning btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir réinitialiser le statut A Payé de tous les joueurs ?');">Réinitialiser A Payé</a>
												</div>
												...
												<?php if ($mode === 'sequential'): ?>
													<div class="alert alert-info" style="max-width:900px;margin:10px auto 0;">
														...
														<a href="finale.php?mode=sequential&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=<?php echo $sortBy; ?>&amp;seq_reset=1" class="btn btn-default btn-sm" style="margin-left:8px;">Réinitialiser l'ordre d'arrivée</a>
														<a href="finale.php?mode=sequential&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=<?php echo $sortBy; ?>&amp;seq_undo=1" class="btn btn-default btn-sm" style="margin-left:8px;">Annuler dernière arrivée</a>
														<a href="finale.php?mode=sequential&amp;equilibrage=<?php echo (int)$autoBalance; ?>&amp;sort=<?php echo $sortBy; ?>&amp;release_all_seats=1" class="btn btn-default btn-sm" style="margin-left:8px;" onclick="return confirm('Êtes-vous sûr de vouloir libérer tous les sièges sauf l\'organisateur ?');">Libérer tous les sièges sauf l'organisateur</a>
													</div>
												<?php endif; ?>
											<?php endif; ?>
										</div>
...
$(function() {
	...
	(function monitoringPolling() {
		...
		function checkForPaymentChanges() {
			fetch('finale.php?finale_mode=1&check_payments=1', {
				method: 'GET',
				headers: {
