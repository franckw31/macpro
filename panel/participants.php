    ...
    if(!empty($con)){
        ...
        if($pq){
            while($row = mysqli_fetch_assoc($pq)){
                ...
                'tombolas' => isset($row['tombolas']) ? intval($row['tombolas']) : 0,
...
(function(){
  ...
  var USER_IS_ADMIN = <?php echo $is_admin ? 'true' : 'false'; ?>;
...
(function(){
    ...
    function render(arr){
        ...
        listEl.innerHTML = arr.map((p,i)=>{
            ...
            // Tombolas checkbox: enabled only for organizer or admin
            var tombAttr = (USER_IS_ORGANIZER || USER_IS_ADMIN) ? `onchange="toggleTombolas(${p.mid}, ${ACT_ID}, this)"` : 'disabled';
            const tombolasChk = `<label title="Tombolas" style="display:inline-block;margin-left:8px"><input type=\"checkbox\" ${p.tombolas==1 ? 'checked' : ''} ${tombAttr} /></label>`;
            ...
            return `<div class="item" role="listitem"><div class="left"><img class="p-avatar" src="${avatarSrc}" alt="" onerror="this.src='https://viendez.com/images/noprofil.jpg'">${pseudoEl}</div><div style="display:flex;align-items:center;gap:8px"><div class="muted">${escapeHtml(formattedDate)}${tombolasChk}</div><div class="accent">${escapeHtml(jet)}</div>${trakBtn}</div></div>`;
        }).join('');
    }
