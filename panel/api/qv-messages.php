...
try {
    ...
    if ($action === 'fetch') {
        ...
        if ($has_dest) {
            ...
            if ($q) while ($row = mysqli_fetch_assoc($q)) {
                $msgs[] = ['id'=> (int)$row['id'], 'from'=> htmlspecialchars($row['pseudo_exp'],ENT_QUOTES,'UTF-8'), 'from_id'=> (int)$row['id_expediteur'], 'role'=> $row['role'], 'mine'=> ((int)$row['id_expediteur'] === $me), 'msg'=> htmlspecialchars($row['message'],ENT_QUOTES,'UTF-8'), 'at'=> $row['created_at'], 'unread'=> ($has_lu ? !((bool)$row['lu_to_recipient']) : false)];
            }
            if ($has_lu) {
            ...
            $q = mysqli_query($con, $sql);
            if ($q) while ($row = mysqli_fetch_assoc($q)) {
                $msgs[] = ['id'=> (int)$row['id'], 'from'=> htmlspecialchars($row['pseudo_exp'],ENT_QUOTES,'UTF-8'), 'from_id'=> (int)$row['id_expediteur'], 'role'=> $row['role'], 'mine'=> true, 'msg'=> htmlspecialchars($row['message'],ENT_QUOTES,'UTF-8'), 'at'=> $row['created_at'], 'unread'=> false];
            }
        }
        echo json_encode(['ok'=>true,'msgs'=>$msgs,'my_role'=>$my_role,'unread'=>$unread,'organizer_id'=>$organizer,'has_dest'=>$has_dest]);
        exit;
    }

    if ($action === 'send') {
        $msg = trim($body['message'] ?? $_POST['message'] ?? '');
        if (!$msg || mb_strlen($msg) > 2000) { echo json_encode(['ok'=>false,'err'=>'invalid_msg']); exit; }
    ...
    if ($action === 'delete') {
        ...
        if ($has_dest) {
            ...
            if ((int)$m['id_expediteur'] !== $me && (int)$m['id_destinataire'] !== $me) { echo json_encode(['ok'=>false,'err'=>'forbidden']); exit; }
        } else {
            $mq = mysqli_query($con,'SELECT id,id_expediteur FROM qv_messages WHERE id='.intval($mid).' LIMIT 1');
            if (!$mq || mysqli_num_rows($mq)===0) { echo json_encode(['ok'=>false,'err'=>'not_found']); exit; }
            $m = mysqli_fetch_assoc($mq);
            if ((int)$m['id_expediteur'] !== $me) { echo json_encode(['ok'=>false,'err'=>'forbidden']); exit; }
        }
    ...
    echo json_encode(['ok'=>false,'err'=>'unknown_action']); exit;
