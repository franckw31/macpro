...
try {
    ...
    // Allow token either in Authorization: Bearer <token> or as form field 'token'
    $authHeader = $_SERVER['HTTP_AUTHORIZATION'] ?? '';
    $token = '';
    if (preg_match('/Bearer\s+(\S+)/', $authHeader, $m)) { $token = $m[1]; }
    if (empty($token) && isset($_POST['token'])) { $token = trim($_POST['token']); }
    if (empty($token)) { echo json_encode(['success'=>false,'error'=>'token manquant']); exit; }

    $stmt = $pdo->prepare("SELECT membre_id, expires_at FROM app_auth_tokens WHERE token = ? LIMIT 1");
    $stmt->execute([$token]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row || strtotime($row['expires_at']) <= time()) { echo json_encode(['success'=>false,'error'=>'token invalide']); exit; }
