...
@MainActor
private final class LiveTimerViewModel: NSObject, ObservableObject {
    ...
    func playTestAudio() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let utterance = AVSpeechUtterance(string: "Ceci est un message de test depuis le live timer.")
    ...
    func playLevelChange(nextBlinds: String) {
        ...
        if lowered.contains("pause") || lowered.contains("break") {
            text = "C'est la pause"
        } else {
            text = "Attention, changement de blindes, \(nextBlinds)"
        }
        ...
        tickTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
            ...
            Task { @MainActor in
                if !self.isPaused && self.secondsRemaining > 0 {
                    ...
                    // Removed playLevelChange here, checking on sync() instead
                }
            }
        }
    }
    ...
    private func sync() async {
        guard let url = URL(string: "https://viendez.com/panel/timer-api.php?uid=\(activityId)") else { return }
        do {
            let (data, _) = try await URLSession.shared.data(from: url)
            let resp = try JSONDecoder().decode(TimerAPIResponse.self, from: data)
            let newLevelName = resp.level_name ?? "Niveau --"
            let newBlindsText = resp.blinds_text ?? "--/--"

            DispatchQueue.main.async { [weak self] in
                guard let self = self else { return }

                if let oldLevelName = self.lastLevelName, 
                   oldLevelName != newLevelName,
                   oldLevelName != "Niveau --" {
                    self.playLevelChange(nextBlinds: newBlindsText)
                }
                self.lastLevelName = newLevelName

                self.status = resp.status
                self.isPaused = resp.is_paused ?? false
                self.secondsRemaining = max(0, resp.seconds_remaining ?? 0)
                self.durationSeconds = max(1, resp.duration_seconds ?? 1)
                self.blindsText = newBlindsText
                self.anteText = resp.ante_text ?? ""
                self.levelName = newLevelName
                self.nextBlindsText = resp.next_blinds_text ?? ""
                self.nextPause = resp.next_pause ?? ""
                self.playersActive = resp.players_active ?? 0
                self.playersTotal = resp.players_total ?? 0
                self.avgStack = resp.avg_stack ?? ""
            }
        } catch {
                ...
                HStack(spacing: 20) {
                    ...
                    Button {
                        ...
                        ZStack(alignment: .bottomTrailing) {
                            Image(systemName: showCountdown ? "stopwatch.fill" : "stopwatch")
                                .font(.title3)
