...
enum KeychainHelper {
    private static let service = "com.cardeventpro.auth"
...
final class AuthService: ObservableObject {
    ...
    @Published var isOrganizer: Bool = false  // 🆕 Rôle organisateur Pro
    ...
    private let organizerKey = "auth.isOrganizer"  // 🆕
    ...
    func autoLogin() async {
        ...
        do {
            ...
            if ok {
                ...
                isOrganizer  = KeychainHelper.read(forKey: organizerKey) == "1"
    ...
    func login(username: String, password: String) async {
        guard !username.isEmpty, !password.isEmpty else { errorMessage = "Veuillez remplir tous les champs"; return }
        isLoading     = true
        errorMessage  = nil
        defer { isLoading = false }
        ...
        let payload: [String: Any] = [ "action": "login", "username": username, "password": password, "device_id": deviceId() ]
        ...
        do {
            ...
            guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any], let success = json["success"] as? Bool, success, let token = json["token"] as? String, let name = json["pseudo"] as? String else {
            ...
            pseudo = name; isAdmin = adminFlag == "1"; isOrganizer = orgFlag == "1"; isAuthenticated = true
        } catch { errorMessage = "Impossible de joindre le serveur" }
    }

    func logout() async {
        if let token = KeychainHelper.read(forKey: tokenKey) { try? await sendLogout(token: token) }
        clearCredentials()
    }

    private func verifyToken(_ token: String) async throws -> Bool {
        guard let url = URL(string: authURL) else { return false }
        var request = URLRequest(url: url); request.httpMethod = "POST"; request.setValue("application/json", forHTTPHeaderField: "Content-Type"); request.timeoutInterval = 6
    ...
    private func sendLogout(token: String) async throws {
        guard let url = URL(string: authURL) else { return }
        var request = URLRequest(url: url); request.httpMethod = "POST"; request.setValue("application/json", forHTTPHeaderField: "Content-Type"); request.timeoutInterval = 5
    ...
    private func clearCredentials() {
        KeychainHelper.delete(forKey: tokenKey); KeychainHelper.delete(forKey: pseudoKey); KeychainHelper.delete(forKey: adminKey); KeychainHelper.delete(forKey: organizerKey); pseudo = ""; isAdmin = false; isOrganizer = false; isAuthenticated = false
    }

    private func jsonError(from data: Data) -> String? {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any], let err = json["error"] as? String else { return nil }
        return err
    }
