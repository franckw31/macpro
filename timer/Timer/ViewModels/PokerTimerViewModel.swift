...
struct Participant: Identifiable {
    ...
    var statutColor: String {
        ...
        case "Inscrit":             return "blue"
        case "Confirmé", "Confirme": return "cyan"
        case "Réservation", "Reservation": return "orange"
...
final class PokerTimerViewModel: ObservableObject {
    ...
    func fetchRegistrationStatus() async {
        guard let token = AuthService.shared.token,
              ...
              let url = URL(string: registerAPIURL + "?activity_id=\(actId)") else { return }
        var request = URLRequest(url: url)
    ...
    func toggleRegistration() async {
        guard !isRegistering,
              ...
              let actId = nextActivityId,
              let url = URL(string: registerAPIURL) else { return }
        isRegistering = true
        ...
        let payload: [String: Any] = ["action": "toggle", "activity_id": actId]
                ...
                if delta != 0 {
                    ...
                    if activityList.indices.contains(selectedActivityIndex) {
                        ...
                        activityList[selectedActivityIndex] = ActivitySummary(
                            ...
                            buyin: old.buyin, rake: old.rake, count: max(0, old.count + delta), startDate: old.startDate
                        )
                    }
                }
                print("Registration toggled → \(registered)")
            }
        } catch {
            print("toggleRegistration error: \(error)")
        }
    }
    ...
    func registerWithOptions(anonyme: Bool, option: Bool, latereg: Bool) async {
        ...
        do {
            ...
            print("registerWithOptions error: \(error)")
        }
    }
    ...
    func fetchParticipants(activityId: Int? = nil) async {
        ...
        do {
            ...
            self.participants = list.map { p in
                Participant(
