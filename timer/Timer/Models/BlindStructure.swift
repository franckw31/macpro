...
final class BlindStructureStore: ObservableObject {
    ...
    private init() {
        ...
        if structures.isEmpty {
            ...
            // Migration : ajouter les structures par défaut manquantes
            var changed = false
    ...
    func load() {
        guard let data = UserDefaults.standard.data(forKey: key),
              let decoded = try? JSONDecoder().decode([BlindStructure].self, from: data)
        else { return }
        structures = decoded
    }

    func add(_ structure: BlindStructure) {
        structures.append(structure)
        save()
    }

    func update(_ structure: BlindStructure) {
        guard let idx = structures.firstIndex(where: { $0.id == structure.id }) else { return }
        structures[idx] = structure
        save()
    }

    func delete(at offsets: IndexSet) {
        structures.remove(atOffsets: offsets)
        if structures.isEmpty {
            structures = BlindStructure.defaultStructures
        }
        save()
