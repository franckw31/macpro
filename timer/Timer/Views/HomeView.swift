...
private final class MiniLiveTimerVM: ObservableObject {
    ...
    private func sync() async {
        guard activityId > 0,
              let url = URL(string: "https://viendez.com/panel/cardevent-api.php?uid=\(activityId)") else { return }
        do {
        ...
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(alignment:.firstTextBaseline, spacing:6) {
                    ...
                    Text("v\(Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.0")").font(.caption).foregroundColor(.white.opacity(0.45))
                }
        ...
        ZStack(alignment: .bottomTrailing) {
            Button { showLiveTimer = true } label: {
                VStack(spacing:6) {
                    ...
                    else if miniTimer.status == "idle" || miniTimer.levelName.isEmpty { Image(systemName: "cardevent").font(.system(size:22)).foregroundColor(cyan).shadow(color: cyan.opacity(0.5), radius:6) }
                    else {
